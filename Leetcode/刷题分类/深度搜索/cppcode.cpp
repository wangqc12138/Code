...
public:
    ...
    void dfs(vector<vector<int>> &next, vector<bool> &visit, int n)
    { // 使用引用比传值快的多！
        for (int i : next[n])
        {
            if (visit[i])
...
{
public:
    bool help(vector<vector<int>> &graph, map<int, int> &visit, int i, int color)
    {
        if (visit.count(i))
        {
            return visit[i] == color;
        }
        visit[i] = color;
        for (auto j : graph[i])
        {
            if (!help(graph, visit, j, !color))
            {
                return false;
            }
        }
...
public:
    ...
    void dfs(vector<int> &candidates, int target, int index)
    {
        ...
        for (int i = index; i < candidates.size(); i++)
        {
            ...
            { // 本轮选择过的不再重复，与排列不同的是，从index开始选择的，前面的自然排除
                continue;
            }
            temp.emplace_back(candidates[i]);
            visit[i] = 1;
...
public:
    ...
    {
        if (s.size() - index > 3 * (4 - temp.size()))
        { // 如果尾巴大于指定值，则直接返回，超出长度
            return;
        }
        if (index >= s.size())
        {
